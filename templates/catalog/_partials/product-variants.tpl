  ...
  {foreach from=$groups key=id_attribute_group item=group}
    ...
    {else if $group.group_type == 'radio'}
      ...
      {block name='product_variants_radio'}
        <div id="group_{$id_attribute_group}" class="group-container">
          {foreach from=$group.attributes key=id_attribute item=group_attribute}
            <div class="input-container">
              <input
                ...
                name="group[{$id_attribute_group}]"
                value="{$id_attribute}"
                id="radio-{$id_attribute}"
                {if $group_attribute.selected} checked="checked"{/if}
              >
              <label for="radio-{$id_attribute}" class=" {if $group_attribute.selected}input-container-selected{/if}">{$group_attribute.name}</label>
            </div>
          {/foreach}
        </div>
      {/block}
